...
interface TitleProps {
  rows: TableRow[];
  className?: string;
  onChange: (value: TableRow[]) => void | undefined;
  addRow: () => void;
  setRows: (value: TableRow[]) => void;
  deleteRow: (id: number) => void;
}

const ParamsTable: React.FC<TitleProps> = ({
  rows,
  onChange,
  addRow,
  deleteRow,
  ...
  const sendParamsValues = (row: TableRow) => {
    ...
    return (
      <div className="flex flex-col h-full">
        ...
        <div className="flex flex-col gap-3 flex-grow items-center justify-center">
          ...
          <button className="button rounded-[16px]" onClick={addRow}>Agregar</button>
        </div>
